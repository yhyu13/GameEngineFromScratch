...
namespace My {
    ...
    class com_exception : public std::exception {
        public:
            explicit com_exception(HRESULT hr) : m_result(hr) {};
            virtual const char* what() const override {
                static char s_str[64] = {0};
                sprintf_s(s_str, "Failure with RESULT of %08X", static_cast<unsigned int>(m_result));
                return s_str;
            }
        private:
            HRESULT m_result; 
...
void WaitForPreviousFrame() {
    // Waiting for the frame to complete before continuing is not the best practice
...
void InitPipeline() {
    ...
    // Load the shaders
#if defined(_DEBUG)
    // Enable better shader debuggin with the graphics debugging tools
...
void InitGraphics() {
    ...
    ThrowIfFailed(g_pVertexBuffer->Map(
        0, 
        &readRange,
...
void PopulateCommandList()
{
    ...
    // list, that commmand list can then be reset at any time and must be before
    // re-recording.
                   ...
                   int nCmdShow)
{
    ...
    try
    {
        ...
        RECT wr;
        wr.left = 0;
        wr.right = nScreenWidth + wr.left;
        wr.top = 0;
        wr.bottom = nScreenHeight + wr.top;
        ...
        hWnd = CreateWindowEx(0,
                            ...
                            (horizontal - true_width) / 2,                                 // x-position of the window
                            (vertical - true_height) / 2 ,                                 // y-position of the window
                            true_width,                         // width of the window
                            true_height,                        // height of the window
